...
def run_stacking(robot_points,cube_names):
    ...
    def pick_and_place(cube_name):
        ...
        if stack_counter>=len(place_z_levels):
            ...
            return
        # get matching position or fall back to the slot index
    pos = next((p for p in robot_points if p["name"]==cube_names), None)
    if pos is None:
        # fallback: use the slot at the same index as we’re stacking
        if stack_counter < len(robot_points):
            pos = robot_points[stack_counter]
        else:
            messagebox.showerror("Error", f"No robot position available for {cube_names}")
            return
