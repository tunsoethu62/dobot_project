...
# ---------- CAMERA INDEX ----------
camera_index = 0  # Change this if your camera index changes
...
if __name__=="__main__":
    ...
    # --- AFTER FIRST STACK ---
    messagebox.showinfo("Reassign Cubes", "First stack done. Camera will open only when detecting newly placed cubes.")
    time.sleep(2)
    ...
    if centroids:
        ...
        new_cube_names = [p["name"] for p in robot_points]
        mapping_info = "\n".join([f"Position {i}: {name}" for i, name in enumerate(new_cube_names)])
        messagebox.showinfo("New Cube Positions", mapping_info)
        run_stacking(robot_points, new_cube_names)
